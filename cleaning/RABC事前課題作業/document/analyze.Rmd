...
joined_data <- readRDS("/Users/ki/Desktop/RABC事前課題作業/data/cleaned/refusal_joined.obj")
...
#図5
joined_data  %>% filter(year == 2022) %>% mutate(prefecture = fct_reorder(prefecture, refusal_rate)) %>% ggplot(aes(y = refusal_rate, x = prefecture)) + geom_col()